...
std::ostream& Logger::start(log_rank_t log_rank, const string& file,int32 line,const std::string &function) {
    time_t tm;
    time(&tm);
    char time_string[64];
    ctime_r(&tm, time_string);
    return getStream(log_rank)<< time_string<<file<<", function:("<< function<<")"<<", line:"<<line<<" "<<std::flush;
}
